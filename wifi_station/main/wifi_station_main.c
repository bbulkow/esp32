...
wifi_ap_info_t *wifi_multi_find(const uint8_t *ssid);


const char *get_authmode_str(int authmode_id) {
    switch(authmode_id) {
        case WIFI_AUTH_OPEN:
...
// returns size copied
inline int u8cmp( const uint8_t *s1, const uint8_t *s2) {
    return( strcmp( (const char *)s1, (const char *) s2));
}

// returns pointer to destination
inline char *u8cpy( uint8_t *dst, const uint8_t *src) {
